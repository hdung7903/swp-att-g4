...
public class EditInformationController extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        boolean hasEdit = accDAO.editInformation(id, username, password,
                Integer.parseInt(type), fullname, email,
                dob, Integer.parseInt(gender));
        request.setAttribute("result", hasEdit);
        request.setAttribute("mess", hasEdit ? "Edit successful" : "Edit failed");
        request.getRequestDispatcher("editInfor.jsp").forward(request, response);
    }
    }
